...
export default function CheckoutPage() {
  ...
  return (
    <div className="page-section-stack">
      ...
      <Card className="section-card"><div className="checkout-bar"><Typography.Text strong className="price-text">应付总额：{formatPrice(totalAmount)}</Typography.Text><Button type="primary" loading={submitting} disabled={submitting} onClick={async function () { if (submitting) return; try { setSubmitting(true); if (!selectedAddressId) { message.warning('请选择收货地址'); return; } const order = createOrder({ userId: user.id, addressId: selectedAddressId, items: draft.items, source: draft.source }); message.success('订单提交成功'); navigate('/pay/' + order.id); } catch (error) { message.error(error.message || '订单提交失败'); } finally { setSubmitting(false); } }}>提交订单</Button></div></Card>
    </div>
  );
}
